...
class XpraClient {
  ...
  init_state() {
    ...
    //this.taskbar_height = w96.ui.Theme.uiVars.taskbarHeight;
    this.taskbar_height = 0;

    const me = this;
    //const screen_element = jQuery("#maingfx");
    //screen_element.mousemove((e) => this.on_mousemove(e));

    const div = document.querySelector("#maingfx");
  ...
  initialize_workers() {
    ...
    const worker = new Worker(XPRA_PREFIX + "lib/wsworker_check.js");
    ...
    if (this.offscreen_api) {
      ...
      decode_worker = new Worker(XPRA_PREFIX + "OffscreenDecodeWorker.js");
    } else {
      this.clog("using decode worker");
      decode_worker = new Worker(XPRA_PREFIX + "DecodeWorker.js");
    }
  ...
  _screen_resized(event) {
    ...
    if (
      ...
      this.container.clientHeight - this.taskbar_height == this.desktop_height
    ) {
      return;
    }
    this.desktop_width = this.container.clientWidth;
    this.desktop_height = this.container.clientHeight - this.taskbar_height;
    ...
    for (const index in this.id_to_window) {
      ...
      if (
        ...
        if (win.fullscreen === false && pattern.test(win.metadata.title)) {
          x11log(`auto fullscreen window: ${win.metadata.title}`);
      ...
      if (this.fullscreen === false && this.client.is_window_desktop(win)) {
        x11log(`auto fullscreen desktop window: ${this.metadata.title}`);
  ...
  query_keyboard_map() {
    ...
    keyboard.getLayoutMap().then((keyboardLayoutMap) => {
      x11log("got a keyboard layout map:", keyboardLayoutMap);
      x11log("keys:", [...keyboardLayoutMap.keys()]);
      for (const key of keyboardLayoutMap.keys()) {
        const value = keyboardLayoutMap[key];
        x11debug("keyboard", key, "=", value);
  ...
  do_keyb_process(pressed, event) {
    ...
    if (this.clipboard_enabled && x11client.clipboard_direction !== "to-server") {
  ...
  _get_screen_sizes() {
    ...
    const screen_size = [
      ...
      this.container.clientHeight - this.taskbar_height,
    ];
  ...
  do_window_mouse_click(e, win, pressed) {
    ...
    let send_delay = 0;
    if (x11client.clipboard_direction !== "to-server" && this._poll_clipboard(e)) {
  ...
  init_clipboard() {
    window.addEventListener("paste", (e) => {
      ...
      if (navigator.clipboard && navigator.clipboard.readText) {
        ...
        x11debug("clipboard", "paste event, data=", paste_data);
        this.clipboard_buffer = paste_data;
        this.send_clipboard_token(paste_data);
      }
    });
    ...
    $("#maingfx").on("click", (e) => this.may_set_clipboard());
    $("#maingfx").keypress(() => this.may_set_clipboard());
  }
  ...
  read_clipboard_text() {
    ...
    x11client.debug("clipboard", "read_clipboard_text()");
  ...
  toggle_window_preview(init_callback) {
    ...
    const windows_sorted = Object.values(x11client.id_to_window).filter((win) => {
      // skip DESKTOP type windows.
      return !x11client.is_window_desktop(win);
    });
  ...
  _handle_window_list_blur(e) {
    if ($(WINDOW_PREVIEW_SELECTOR).is(":visible")) {
      ...
      x11client.toggle_window_preview();
    }
  }
  ...
  process_xdg_menu() {
    ...
    this.on_xdg_menu(this.xdg_menu);
    return;
  }
  ...
  send_tray_configure(wid) {
    const div = jQuery(this.id_to_window[wid].div);
    const x = Math.round(div.offset().left);
    const y = Math.round(div.offset().top);
    const w = TRAY_SIZE;
    const h = TRAY_SIZE;
    this.id_to_window[wid].handle_moved();
  ...
  suspend() {
    const window_ids = Object.keys(x11client.id_to_window).map(Number);
  ...
  _new_window(wid, x, y, w, h, metadata, override_redirect, client_properties) {
    ...
    const screen = document.querySelector("#maingfx");
    screen.append(mydiv);
  ...
  send_file(f) {
    x11log("send_file:", f.name, ", type:", f.type, ", size:", f.size);
    const fileReader = new FileReader();
